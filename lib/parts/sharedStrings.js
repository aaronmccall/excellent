...
assign(SharedStrings.prototype, {
    ...
    addRich: function (obj) {
        var matches = this.rich.filter(function (potential) {
            return isEqual(obj, potential.obj);
        });
        if (matches.length) {
            return this.stringMap[matches[0].id];
        }
        var id = uuid.v4();
        var index = this.strings.length;
        this.strings.push(obj);
        this.rich.push({obj: obj, id: id});
        return (this.stringMap[id] = index);
    }
});
