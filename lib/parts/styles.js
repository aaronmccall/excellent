...
assign(Styles.prototype, {
    addDirective: function (type, config) {
        ...
        var typeParser;
        if ((typeParser = this['parse' + utilities.upperFirst(type)])) {
            parsed = typeParser.call(this, parsed);
        }
        container.push(parsed);
        var refs = this[this.directiveToRefs(type)];
        var indexer;
        refs[config.label] = (indexer = this[type + 'Indexer']) ? indexer(index) : index;
    },
    ...
    getCellStyle: function (label) {
        return this.cellStyleRefs[label];
    },
    getFont: function (label) {
        var fontIdx = this.fontRefs[label];
        return this.fonts[fontIdx];
    },
    numberFormatsIndexer: function (index) {
        return 200 + index;
    },
    ...
    parseBorders: function (config) {
        each(config, function (def, key) {
            if (key !== 'label') {
                config[key] = this.parseColors(def);
            }
        }, this);
        return config;
    },
...
Object.defineProperties(Styles.prototype, utilities.defineProps({
    ...
    unsettable: {
        ...
        fontRefs: refsCtor,
        fonts: { ctor: function () { return [{label: 'none',size: 12, name: 'Calibri', color: {indexed: 64}}];} },
